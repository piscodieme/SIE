...
export default function TableauAutres(props) {
    ...
    const deleteAutres =(id)=>{
        industrieService.deleteAutres(id).then((res) =>{
   ...
   <div>
    {
                                            ...
                                            <button type="button" className='myButton' onClick={()=>deleteAutres(idProd)}>
                                            Supprimer
                                            </button>
                                            ...
                                            }
    {
        ...
        <table className='table table-striped table-bordered mt-1'>
                            <thead>
                                ...
                                <th className='text-center'>Production total</th>
                                <th className='text-center'>Consommation total</th>
                                <th className='text-center'>Vente total</th>
                                <th className='text-center'>Achat total</th>
                            ...
                            <tbody className=''>
                                {
                                   line && line.map(
                                        (prod,i) => <tr key={prod.id} className='text-center paddCell'>
                                            <td >{i+1}</td>
                                            <td>{prod.annee}</td>
                                            <td>{prod.prodTotal}</td>
                                            <td>{prod.consoTotal}</td>
                                            <td>{prod.venteElec}</td>
                                            <td>{prod.achatElec}</td>
                                            <td>{prod.unite}</td>
                                            <td>
