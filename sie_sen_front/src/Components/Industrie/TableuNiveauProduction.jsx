...
export default function TableauNiveauProduction(props) {
    ...
    const deleteNiveau =(id)=>{
        industrieService.deleteNiveauProduction(id).then((res) =>{
            console.log(res.data);
            setNotif(true)
            setMsg("Suppression réussie ")
            window.location.reload();
        }).catch((err)=>{
            console.log("erreur delete niveau production",err)
            setNotifErreur(true)
   ...
   <div>
    {
                                            ...
                                            <button type="button" className='myButton' onClick={()=>deleteNiveau(idProd)}>
                                            Supprimer
                                            </button>
                            ...
                            <tbody className=''>
                                {
                                   line && line.map(
                                        (prod,i) => <tr key={prod.id} className='text-center paddCell'>
                                            <td >{i+1}</td>
                                            <td className='text-center'>{prod.nomProduit}</td>
                                            <td>{prod.annee}</td>
                                            <td>{prod.unite}</td>
                                            <td>{prod.quantite}</td>
                                            <td>
